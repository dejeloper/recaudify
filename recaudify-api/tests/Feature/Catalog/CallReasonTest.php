...
class CallReasonTest extends TestCase
{
    ...
    public function test_destroy_and_restore_call_reason(): void
    {
        ...
        $this->actingAs($this->user, 'api')->deleteJson("/api/call-reasons/{$reason->id}")->assertStatus(200);
        $this->assertSoftDeleted('call_reasons', ['id' => $reason->id]);

        $this->actingAs($this->user, 'api')->postJson("/api/call-reasons/{$reason->id}/restore")->assertStatus(200);
