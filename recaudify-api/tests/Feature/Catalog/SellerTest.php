...
class SellerTest extends TestCase
{
    ...
    public function test_destroy_and_restore_seller(): void
    {
        ...
        $this->actingAs($this->user, 'api')->deleteJson("/api/sellers/{$seller->id}")->assertStatus(200);
        $this->assertSoftDeleted('sellers', ['id' => $seller->id]);

        $this->actingAs($this->user, 'api')->postJson("/api/sellers/{$seller->id}/restore")->assertStatus(200);
