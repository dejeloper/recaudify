...
class RateTest extends TestCase
{
    ...
    public function test_index_lists_rates(): void
    {
        ...
        Rate::create($this->rateData(['name' => '10 Cuota - Biblia', 'installments' => 10, 'installment_value' => 35000]));

        $response = $this->actingAs($this->user, 'api')->getJson('/api/rates');

        $response->assertStatus(200)->assertJsonCount(2, 'data')->assertJsonPath('data.0.product.name', 'Biblia Grande');
    }
    ...
    {
        $response = $this->actingAs($this->user, 'api')->postJson('/api/rates', $this->rateData(['product_id' => 9999]));
    ...
    public function test_destroy_and_restore_rate(): void
    {
        ...
        $this->actingAs($this->user, 'api')->deleteJson("/api/rates/{$rate->id}")->assertStatus(200);
        $this->assertSoftDeleted('rates', ['id' => $rate->id]);

        $this->actingAs($this->user, 'api')->postJson("/api/rates/{$rate->id}/restore")->assertStatus(200);
